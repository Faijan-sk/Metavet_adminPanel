// File: components/WalkerKycList.jsx
    ...
    useEffect(() => {
        const fetchKycs = async () => {
            try {
                setLoading(true)
                setError('')
                const res = await jwt.getAllKycMetavetToWalker()
        ...
        <>
            <TableContainer component={Paper} sx={{ maxHeight: 440 }}>
                <Table stickyHeader aria-label="walker kyc table">
                    <TableHead>
                        <TableRow>
                            <TableCell>Walker Name</TableCell>
                    ...
                    <TableBody>
                        {loading ? (
                            ...
                            visibleKycs.map((item, idx) => {
                                ...
                                const uid = item?.uid || item?.id || item?.email || item?.phone || `${item?.businessName || 'walker'}-${idx}`
                                const key = uid
