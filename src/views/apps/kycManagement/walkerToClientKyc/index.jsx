    ...
    useEffect(() => {
        const fetchKycs = async () => {
            try {
                ...
                // res options:
                // 1) service returns axios response      -> res.data = { code, data: { totalRecords, records }, ... }
                // 2) service already returns res.data    -> res = { code, data: { totalRecords, records }, ... }

                const root = res?.code && res?.data ? res : res?.data ?? res

                const dataWrapper = root?.data ?? root

                const list = Array.isArray(dataWrapper?.records)
                    ? dataWrapper.records
                ...
                // Debugging ke liye:
                // console.log('Walker KYC root:', root)
                // console.log('Walker KYC list:', list)

                setKycs(list)
            } catch (err) {
                console.error('Error fetching Walker-to-Client KYC records:', err)
                    ...
                    <TableBody>
                        {loading ? (
                            ...
                            visibleKycs.map(item => {
                                ...
                                const signatureDate =
                                    item?.signatureDate || item?.signature_date || '—'
